...
/* 재사용 유틸: foreign.js와 동일 — 필요 함수만 복사 */
...
export default function IndustryAIKoreaPage() {
  ...
  async function load(d = days, l = limit) {
    try {
      ...
      const blt = (Array.isArray(aiData?.bullets) && aiData.bullets.length) ? aiData.bullets : deriveBulletsFromSummary(sum);
              ...
              <ol className="refs-list">
                {items.map((it, idx)=>(
                  <li id={`ref-${idx+1}`} key={idx} className="refs-item">
                    <a href={it.url} target="_blank" rel="noreferrer" className="link">{it.title}</a>
