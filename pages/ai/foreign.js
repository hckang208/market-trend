...
export default function AIForeignNews() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      setError(null);
      try {
        const r = await fetch(`/api/ai-news-foreign?ts=${Date.now()}`, { signal: abort.signal, cache: "no-store" });
  ...
  return (
    <main style={{maxWidth:900, margin:"0 auto", padding:"20px"}}>
      <h1 style={{margin:"0 0 12px 0"}}>해외 산업 뉴스 · AI 요약</h1>
