...
function topKeywordsFromData({ summary, items }, n = 8) {
  const pools = [];
  if (summary) pools.push(summary);
  if (Array.isArray(items)) pools.push(items.map(i => i.title).join(" "));
...
function highlightInNode(root, terms) {
  ...
  const walker = document.createTreeWalker(root, NodeFilter.SHOW_TEXT, {
    acceptNode(node) {
      ...
      if (node.parentElement && node.parentElement.closest('mark[data-hl="1"]')) return NodeFilter.FILTER_REJECT;
...
export default function IndustryAIForeignPage() {
  ...
  const [items, setItems] = useState([]); // 참조 뉴스
  ...
  async function load(d = days, l = limit) {
    try {
      ...
      const [aiData, newsData] = await Promise.all([
        fetchJSON(`/api/ai-news-foreign?days=${d}&limit=${l}`),
        // index.js와 동일 로직으로 레퍼런스 뉴스 수집
        (async () => {
          const data = await getOverseasNews();
          const raw = Array.isArray(data) ? data : (data?.items || []);
          const mapped = raw.map((n) => {
            const host = (() => {
              try { const u = new URL(n.url || n.link || ""); return u.host.replace(/^www\./, ""); }
              catch { return ""; }
            })();
            return {
              title: n.title || "",
      ...
      if (!aiData?.ok) setError(aiData?._meta?.error || "AI 요약 생성 실패");
  ...
  function applyInputKeywords() {
    const fromInput = kwInput.split(/[,\s]+/g).map((s) => s.trim()).filter(Boolean);
    const merged = Array.from(new Set([...kwSelected, ...fromInput]));
    setKwSelected(merged); setKwInput("");
  }
  function toggleChip(word) {
    const w = String(word || "").trim(); if (!w) return;
    setKwSelected((prev) => (prev.includes(w) ? prev.filter((x) => x !== w) : [...prev, w]));
  }
    ...
    <>
      ...
      <div className="container">
        ...
        <div className="company-header">
          <div>
            <h1 className="company-title">
              산업뉴스 AI 분석 <span className="company-title-sub">(해외)</span>
            </h1>
            <div className="meta">
              {articlesUsed ? `${articlesUsed}개 기사` : ""}{articlesUsed ? " · " : ""}
              {generatedAtLocal}
            </div>
          </div>
          <div className="header-actions">
            <a href="/" className="ghost-link" title="대시보드로 이동" aria-label="대시보드로 이동">← 대시보드</a>
        ...
        <div className="controls">
          ...
          <button
            onClick={() => { router.replace(`/ai/foreign?days=${days}&limit=${limit}`); load(days, limit); }}
            disabled={loading}
            className="apply-btn"
            title="현재 설정으로 다시 요약"
          >
        ...
        <div className="hl-bar">
          <div className="hl-left">
            ...
            <div className="hl-inputs">
              <input value={kwInput} onChange={(e) => setKwInput(e.target.value)} placeholder="쉼표(,)로 여러 키워드 입력 (예: margin, 재고, guidance)" />
          ...
          {!!kwSuggest.length && (
            <div className="hl-suggest">
              {kwSuggest.map((w, i) => (
                <button key={i} onClick={() => toggleChip(w)} className={`chip ${kwSelected.includes(w) ? "active" : ""}`} title="추천 키워드">
                  {w}
                </button>
              ))}
            </div>
      ...
      <style jsx>{`
        ...
        .btn { padding: 8px 14px; border-radius: 10px; border: 1px solid #e5e7eb; background: #fff; font-size: 13px; }
