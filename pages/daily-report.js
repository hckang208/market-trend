  ...
  return (
    <>
      ...
      <main style={{ maxWidth: 900, margin: "0 auto", padding: 16 }}>
        <h1 style={{ margin: "8px 0 12px", fontSize: 24, fontWeight: 900 }}>🤖 Hansoll AI Daily Report</h1>
        ...
        {state.data && (
          <>
            <div style={{ color: "#6b7280", fontSize: 12, marginBottom: 8 }}>
              ...
              {state.data?.meta?.indicatorsUpdated ? ` · 지표 업데이트: ${new Date(state.data.meta.indicatorsUpdated).toLocaleString("ko-KR")}` : ""}
            </div>
            <article style={styles.article} dangerouslySetInnerHTML={{ __html: mdToHtml(state.data.narrative) }} />
          </>
        )}
      </main>
    </>
