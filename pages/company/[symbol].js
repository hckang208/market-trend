...
const EN_STOP = new Set([
  "the","a","an","and","or","but","with","of","for","to","in","on","at","by","as","is","are","was","were","be","been",
  "from","that","this","these","those","it","its","they","their","we","our","you","your","i","me","my","us",
]);
...
function highlightInNode(root, terms) {
  ...
  for (const textNode of toProcess) {
    ...
    for (const re of regexes) {
      ...
      changed = true;
      // 새로 생긴 노드들 중 텍스트를 이어서 다른 regex에도 적용하기 위해 마지막 텍스트 노드를 기준으로 업데이트
      currentNode = frag.lastChild && frag.lastChild.nodeType === 3 ? frag.lastChild : null;
      if (!currentNode) break;
    }
    if (changed && textNode.parentNode) textNode.parentNode.normalize();
  }
}
