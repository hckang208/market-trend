...
export default function CompanyNewsSummaryPage() {
  ...
  return (
    <div style={styles.container}>
      ...
      {error && <div className="s-pages-company-symbol-js-auto1">에러: {error}</div>}

      <div style={{display:"flex", gap:12, alignItems:"center", margin:"12px 0"}}>
        <label>개수 <input type="number" min={3} max={20} value={limit} onChange={e => setLimit(Number(e.target.value))} className="s-pages-company-symbol-js-auto2" /></label>
        <label>일수 <input type="number" min={3} max={30} value={days} onChange={e => setDays(Number(e.target.value))} className="s-pages-company-symbol-js-auto3" /></label>
        <label>언어 <select value={lang} onChange={e => setLang(e.target.value)} className="s-pages-company-symbol-js-auto4"><option value="ko">한국어</option><option value="en">English</option></select></label>
      </div>

      {(!data && !loading) && <div>요약을 불러오려면 잠시 기다려 주세요…</div>}

      {data && (
        <div className="s-pages-company-symbol-js-grid">
          <div className="s-pages-company-symbol-js-summary">
            {htmlSections.length === 0 ? (
              <div className="s-pages-company-symbol-js-auto5" dangerouslySetInnerHTML={{ __html: linkifyCitations(data.summary || "").replace(/\n/g, "<br/>") }} />
            ) : (
              htmlSections.map((sec, idx) => (
                <section key={idx} style={{ marginTop: idx===0 ? 0 : 14 }}>
                  {sec.title && <h3 style={styles.h3}>{sec.title}</h3>}
                  <div className="s-pages-company-symbol-js-auto6" dangerouslySetInnerHTML={{ __html: sec.html }} />
                </section>
              ))
            )}
          </div>

          <aside className="s-pages-company-symbol-js-card">
            <div style={styles.refTitle}>참조 뉴스</div>
            <ol className="s-pages-company-symbol-js-auto7">
              {(data.items || []).map((it, idx) => (
                <li id={`ref-${idx+1}`} key={idx} className="s-pages-company-symbol-js-auto8">
